...
//
//BinaryNode<ItemType>* BinarySearchTree<ItemType>::findNode(BinaryNode<ItemType>* subTreePtr,
//	const ItemType& target, int l) const
//{
//	
//	// Uses a binary search 
//	if (subTreePtr == nullptr)
...
//Function for find comparison and search data
bool BinarySearchTree<ItemType>::contains(const ItemType& anEntry,int L) const
{




	bool b= (findNode(rootPtr, anEntry) != nullptr) ? true : false;  // nullptr is same as false
...
template<class ItemType>
bool BinarySearchTree<ItemType>::containsl(const ItemType& anEntry, int L) const//Function for find comparison
{
	bool b = false;
	int s = stoi(anEntry);
	for (int i=s;i<=L;i++)
	{ 
		string anEntryele = {};
		anEntryele = std::to_string(i);
     	 b=(findNode(rootPtr, anEntryele) != nullptr) ? true : false;  // nullptr is same as false
	}
	return b;
